...
export default function HeaderClient({ data }: { data: Header }) {
  ...
  return (
    <>
      <header className={`w-full border-b  ${data.sticky ? 'sticky top-0 z-[90] backdrop-blur bg-white/40' : ''}`}>
      ...
      <div
        ref={panelRef}
        className={`fixed right-0 top-0 z-[100] h-dvh w-80 max-w-[85%] border-l border-neutral-200 bg-white p-6 shadow-xl transition-transform md:hidden ${open ? "translate-x-0" : "translate-x-full"}`}
        role="dialog"
        aria-modal="true"
      >
