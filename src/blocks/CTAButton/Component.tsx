...
type Props = {
    label: string
    link?: AnyLink | null
    variant?: 'primary' | 'secondary' | 'link' | 'solid' | 'bordered' | 'faded' | 'light' | 'ghost' | 'shadow'
    size?: 'xs' | 'sm' | 'md' | 'lg' | 'xl'
...
export default function CTABlockComponent(props: Props) {
    ...
    return (
        <div className={cn('flex', alignWrapper[align])}>
            {href === '#' ? (
                <button className={btnClass} disabled={isDisabled || isLoading} type="button">
                    {ButtonInner}
                </button>
            ) : (
                <Link href={href} target={target} rel={rel} className={btnClass} aria-disabled={isDisabled || isLoading}>
                    {ButtonInner}
                </Link>
            )}
        </div>
